...
const tMenuItem const settingsItems[] PROGMEM = {
    ...
    { text: m_mGHIntegral,      ptr: &ee_settings[eeGHIntegral],        type: mtFloat},
    { text: m_mGHProportional,  ptr: &ee_settings[eeGHProportional],    type: mtFloat},
    { text: m_mGHDerivative,    ptr: &ee_settings[eeGHDerivative],      type: mtFloat},
...
void MainScreen() {
    ...
    uint16_t temp = GunTemp(VarGunHeat);
    if (1024 < temp) temp = 0;
    lcdRamPrint(Str(temp), normal); lcdPrintChar(cCelsius, normal); lcdPrintChar(0x20, normal);

    lcdLocate(1, 0);
    lcdPrint(m_guntemp, normal);
    lcdRamPrint(Str(GunTemp(GunTempSum / GunTemps_size)), normal); lcdPrintChar(cCelsius, normal); lcdPrintChar(0x20, normal);
    if (ShowDebugInfo) {
        lcdRamPrint(Str(GunTempSum / GunTemps_size), normal); lcdPrintChar(0x20, normal);
    }
...
void MenuScreen() {
    ...
    for (int8_t row = 0; row < LCD_ROWS; row++) {
        ...
        if (row+offset < length) {
            ...
            if (mtBoolean == type && *(bool*) ptr) {
                lcdPrintChar(cCheck, style);
                printed++;
            } else if (mtUInteger == type) {
